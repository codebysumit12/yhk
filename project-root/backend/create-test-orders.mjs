...
mongoose.connect('mongodb://localhost:27017/yhk-food-delivery')
  .then(async () => {
    ...
    try {
      // First create a test menu item with unique slug
      const testItem = await MenuItem.create({
        ...
        slug: `test-burger-${Date.now()}` // Unique slug with timestamp
      });
      ...
      const sampleOrders = [
        {
          ...
          paymentMethod: 'cash', // Fixed: use valid enum value
          pricing: {
            subtotal: 300,
            deliveryFee: 30,
            tax: 15,
        ...
        },
        {
          ...
          paymentMethod: 'online', // Fixed: use valid enum value
          pricing: {
            subtotal: 200,
            deliveryFee: 0,
            tax: 10,
